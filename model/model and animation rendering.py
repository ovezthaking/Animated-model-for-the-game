...
def draw_rope(particles, radius=0.02):
    for i in range(len(particles) - 1):
        p1 = particles[i]
        p2 = particles[i + 1]
        draw_cylinder(p1, p2, radius)

def draw_cylinder(p1, p2, radius, slices=20, stacks=20):
    direction = p2 - p1
    ...
    # Create a rotation matrix to align the cylinder with the direction vector
    up = np.array([0, 1, 0])
